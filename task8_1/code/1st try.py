...
def Balls_dataset(IMAGE):
    ...
    for contour in blue_contours:
        ...
        center = (int(x), int(y))
        radius = int(radius)
        if(cv2.contourArea(contour)>max_blue*0.75 and cv2.contourArea(contour)<100000 ):
    ...
    for contour in red_contours:
        ...
        center = (int(x), int(y))
        radius = int(radius)
        if(cv2.contourArea(contour)>max_red*0.75 and cv2.contourArea(contour)<100000):
...
Balls_dataset("b_000")
#extra accuracy code block for the last photo
